...
import {
  eventBaseColor, entityColor, EVENT_TYPE_COLOR, FEDERATION_COLOR, STATE_VISUAL, STATE_LABEL, withAlpha,
      ...
      <div className="space-y-3 rounded-xl border border-border bg-card p-3 shadow-sm">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1.5">
            {(Object.keys(FEDERATION_COLOR) as TournamentFederation[]).map((f) => (
              <span key={f} className="flex items-center gap-1.5 text-xs text-foreground">
                <span className="h-2 w-2 rounded-full" style={{ backgroundColor: FEDERATION_COLOR[f] }} />
                {f}
              </span>
            ))}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {ALL_FEDERATIONS.map((f) => (
            <PlayerFilterChip
              ...
              icon={<span className="h-2 w-2 rounded-full" style={{ backgroundColor: FEDERATION_COLOR[f] }} />}
            />
          ))}
          <Button
            variant="ghost"
